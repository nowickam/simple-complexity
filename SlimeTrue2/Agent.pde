class Agent
{
  ...
  Agent(float x, float y, float rot, int _r, int _g, int _b, color[] _background)
  {
  ...
  void move()
  {
    ...
    if(checkEdge() && attemptMove())
    {
        ...
        //pixels[constrain(round(location.y)+1, 0, height-1)*width+round(location.x)] = color(r,g,b);
        //pixels[constrain(round(location.y)-1, 0, height-1)*width+round(location.x)] = color(r,g,b);
        //pixels[constrain(round(location.y), 0, height-1)*width+constrain(round(location.x)+1, 0, width-1)] = color(r,g,b);
        //pixels[constrain(round(location.y), 0, height-1)*width+constrain(round(location.x)-1, 0, width-1)] = color(r,g,b);
    }
    else
    {
    ...
    for(float i=-SW; i <=SW; i++)
    {
      for(float j=-SW; j<=SW; j++)
      {
         if(round(x+j) >= 0 && round(x+j) < width && round(y+i) >= 0 && round(y+i) < height)
         {
           ...
           sumB += red(background[round(y+i)*width+round(x+j)])+blue(background[round(y+i)*width+round(x+j)])+green(background[round(y+i)*width+round(x+j)]);
         }
      }
    }
